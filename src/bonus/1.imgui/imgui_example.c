...
void setup_buffers_and_shaders() {
  ...
  u_normalMatrix =
      bgfx_create_uniform("u_normalMatrix", BGFX_UNIFORM_TYPE_MAT3, 1);
}
...
int main(int argc, char* argv[]) {
  ...
  // let also do SDL_GetDisplayDPI
  float ddpi, hdpi, vdpi;
  if (SDL_GetDisplayDPI(0, &ddpi, &hdpi, &vdpi) != 0) {
    printf("SDL_GetDisplayDPI failed: %s\n", SDL_GetError());
  } else {
    printf("ddpi: %f, hdpi: %f, vdpi: %f, ddpi/96: %f\n", ddpi, hdpi, vdpi, ddpi/96.0f);
  }

  float sdl_scale = ddpi / 96.0f;
  ...
  while (running) {
    ...
    while (SDL_PollEvent(&event)) {
      ...
      // if left right key pressed rotate the camera with rotate_camera(camrera,
      // angle)
    ...
    // imgui_new_frame(int32_t _mx, int32_t _my, uint8_t _button, int32_t
    // _scroll, uint16_t _width, uint16_t _height, int _inputChar,
    // bgfx_view_id_t _viewId)
    imgui_new_frame(mx, my, button, scroll, width, height, inputChar, 255, sdl_scale);

    // igText("Hello, world!");
    igBegin("Hello, world!", NULL, 0);
    // igSetWindowFontScale(scale);

    // print getMacOSScaleFactor
    igText("getMacOSScaleFactor: %f", sdl_scale);
    // igSetWindowFontScale(scale);
    ...
    igInputText("input text", buf, 128, 0, NULL, NULL);

    igEnd();

    imgui_end_frame(scale);
    ...
    // bgfx_dbg_text_printf(0, 0, 0x4f, "Hello, imgui example!");
    ...
    bgfx_set_uniform(u_objectColor, (vec4){1.0f, 0.5f, 0.31f, 1.0f}, 1);
