...
#include <fs_textures_exercise_4.sc.glsl.bin.h>
#include <vs_textures_combined.sc.glsl.bin.h>
#elif BX_PLATFORM_OSX
#include <fs_textures_exercise_4.sc.mtl.bin.h>
#include <vs_textures_combined.sc.mtl.bin.h>
#elif BX_PLATFORM_WINDOWS
#include <fs_textures_exercise_4.sc.dx11.bin.h>
...
bgfx_uniform_handle_t u_mix_amount;
...
#if BX_PLATFORM_LINUX || BX_PLATFORM_BSD
  shader_program = load_shader_embedded(vs_textures_combined_glsl, sizeof(vs_textures_combined_glsl), fs_textures_exercise_4_glsl, sizeof(fs_textures_exercise_4_glsl));
#elif BX_PLATFORM_OSX
  shader_program = load_shader_embedded(vs_textures_combined_mtl, sizeof(vs_textures_combined_mtl), fs_textures_exercise_4_mtl, sizeof(fs_textures_exercise_4_mtl));
#elif BX_PLATFORM_WINDOWS
  shader_program = load_shader_embedded(vs_textures_combined_dx11, sizeof(vs_textures_combined_dx11), fs_textures_exercise_4_dx11, sizeof(fs_textures_exercise_4_dx11));
#endif

  // Set uniforms
  u_color = bgfx_create_uniform("u_color", BGFX_UNIFORM_TYPE_VEC4, 1);
  u_mix_amount = bgfx_create_uniform("u_mix_amount", BGFX_UNIFORM_TYPE_VEC4, 1);
}
...
int main(int argc, char* argv[]) {
  ...
  SDL_SetWindowTitle(window, "Shaders Textures - Exercise 4");
  ...
  while (running) {
    ...
    bgfx_dbg_text_printf(0, 0, 0x4f, "Hello, Textures - Exercise 4!");

    float greenValue = sin(SDL_GetTicks() * 0.001f);
    float color[4] = {0.0f, greenValue, 0.1f, 1.0f};
    // bgfx_set_uniform(u_color, color, 1);

    float mix_value[4] = {mix_amount, 0.0f, 0.0f, 0.0f};
    bgfx_set_uniform(u_mix_amount, mix_value, 1);
  ...
  bgfx_destroy_uniform(u_mix_amount);
