...
#include <fs_textures_exercise_1.sc.glsl.bin.h>
...
#if BX_PLATFORM_LINUX || BX_PLATFORM_BSD
  shader_program = load_shader_embedded(vs_textures_combined_glsl, sizeof(vs_textures_combined_glsl), fs_textures_exercise_1_glsl, sizeof(fs_textures_exercise_1_glsl));
#elif BX_PLATFORM_OSX
...
int main(int argc, char* argv[]) {
  ...
  SDL_SetWindowTitle(window, "Shaders Textures Combined");
  ...
  bgfx_texture_handle_t awesomeface_texture = create_texture_from_surface(awesomeface_surface);
  ...
  bgfx_uniform_handle_t s_texture_2 = bgfx_create_uniform("s_texture_2", BGFX_UNIFORM_TYPE_SAMPLER, 1);
  ...
  while (running) {
    ...
    bgfx_dbg_text_printf(0, 0, 0x4f, "Hello, Textures Combined!");
