...
void setup_buffers_and_shaders() {
  ...
  shader_program = load_shader_embedded(
      (uint8_t*)vs_transformations_mtl, sizeof(vs_transformations_mtl),
      (uint8_t*)fs_transformations_mtl, sizeof(fs_transformations_mtl));
...
int main(int argc, char* argv[]) {
  ...
  while (running) {
    ...
    glm_scale(transform, (vec3){scaleFactor, scaleFactor, scaleFactor});
    glm_rotate(transform, rotationAngle, rotationAxis);    

    bgfx_set_uniform(u_transformation, &transform, 1);
