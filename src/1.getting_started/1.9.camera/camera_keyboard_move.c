...
void process_camera_movement(const Uint8* key_state) {
  if (key_state[SDL_SCANCODE_W]) {
    vec3 movement;
    glm_vec3_scale(camera_front, camera_speed, movement);
  ...
  if (key_state[SDL_SCANCODE_A]) {
    ...
    glm_vec3_scale(right, camera_speed, right);
...
int main(int argc, char* argv[]) {
  ...
  while (running) {
    ...
    process_camera_movement(key_state);
